...
<link rel="stylesheet"
	href="../bootstrap-4.3.1-dist/cssClases/formulario-inicio-sesion.css">
...
if (isset($_REQUEST["incorrecto"])) {
    echo "<div class='alert alert-danger alert-formato-danger' role='alert'>Usuario o contraseña incorrectos.</div>";
}
if (isset($_REQUEST["sesionCerrada"])) {
    echo "<p><div class='alert alert-success alert-formato-success' role='alert'>Ha salido correctamente. Su sesión está ahora cerrada.</div>";
}
if (isset($_REQUEST["exito"])) {
    echo "<p><div class='alert alert-success alert-formato-success' role='alert'>&Eacute;xito al registrarte, ahora inicia sesi&oacute;n.</div>";
}

?>

	<h1 class="titulo">Acceso a usuarios</h1>
	<div class="contenedorInicioSesion ">


		<form class="formularioRegistro col-12" action="../vivienda-listado.php" method="POST">
			<div class="form-group input-lg">
				<label class="label">EMAIL DE ACCESO</label></br> <input 
					placeholder="Mail@mail.com" type="email" name="identificador"
					required />
			</div>
			<div class="form-group input-lg">
				<label class="label">CONTRASE&Ntilde;A: </label></br> <input  
					type="password" name="contrasenna" required />
			</div>
			<div class="form-group">
				<input type="checkbox" name="recuerdame" /> <label>RECU&Eacute;RDAME</label>
			</div>
			<input class="boton" type="Submit" value="Entrar a HouseFinder" />

		</form>

	</div>
	<p class="texto">
		&iquest;No eres un usuario de HouseFinder? <a
			href="formulario-nuevo-usuario.php">REG&Iacute;STRATE</a>
	</p>
